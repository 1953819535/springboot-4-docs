...
7. **灰度期全站一刀切** —— 开关是应用级全局开关，没有按接口粒度的灰度能力。正确姿势：先在非核心服务开、压测 IO 占比与 pinning 事件，再推广；核心交易链路保留回滚预案（改回 false + 调回池参数）。
8. **JVM 崩溃 / OOM 后归因错位** —— 百万虚拟线程同时持有请求上下文（MDC、事务、缓冲区）时，内存水位与线程数成正比。溢出时先看堆中线程对象与 ThreadLocal 链，不要条件反射调 `-Xmx`。
